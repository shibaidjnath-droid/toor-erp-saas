...
router.get("/", async (_req, res) => {
  try {
    ...
    console.error("DB error (get roles):", err);
    res.status(500).json({ error: "Database error while fetching roles" });
  }
});

/** ✅ POST – nieuwe rol toevoegen */
router.post("/", async (req, res) => {
  try {
    ...
    const check = await pool.query("SELECT * FROM roles WHERE name = $1", [value]);
    ...
    console.error("DB insert error (role):", err);
...
router.delete("/", async (req, res) => {
  try {
    ...
    const result = await pool.query("DELETE FROM roles WHERE name = $1 RETURNING id", [value]);
    if (!result.rowCount) return res.status(404).json({ error: "Role not found" });
    res.json({ ok: true });
  } catch (err) {
    console.error("DB delete error (role):", err);
